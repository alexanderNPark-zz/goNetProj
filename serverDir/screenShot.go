...
//most credit goes to screenshot_windows.go from github api
import (
	"fmt"
	"image"
	"reflect"
...
func ScreenRect() (image.Rectangle, error) {
	...
	x := GetDeviceCaps(hDC, DESKTOPHORZRES)//MUST USE DESKTOP VERSION
	y := GetDeviceCaps(hDC, DESKTOPVERTRES)//NOTE *using HORZRES and VERTRES do not give you the full screen, only setResolution
...
func PNGScreenShotToBytes(writer io.Writer){
	image,err:=CaptureScreen()
	if(err!=nil){
		fmt.Println(err)
	}
...
func GetSystemMetrics(values int) int{
	value,_,_:=procGetSystemMetrics.Call(uintptr(values))
	return int(value)
}
